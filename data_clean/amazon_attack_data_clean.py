...
def attack_data_deal(normal_file,attack_file1,attack_file2,attack_file3,proportion):
    user_item_pairs = np.loadtxt(normal_file, dtype=np.int64, delimiter='\t')
    user_ids = user_item_pairs[:, 0]
    item_ids = user_item_pairs[:, 1]
    ratings = user_item_pairs[:, 2].astype(np.float32)
    num_users = np.max(user_ids) + 1
    num_items = 2753
    rating_matrix = np.zeros((num_users, num_items), dtype=np.float32)
    rating_matrix[user_ids, item_ids] = ratings
